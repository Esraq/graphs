...
class PublicSslCommerzPaymentController extends Controller
{
    ...
    public function index(Request $request) 
    {
           ...
           $z=$request->get('terms');
            ...
            /* $example=new Example;
             $example->email=$email;
             $example->amount=$a;
             $example->payment_type=$payment_type;
             $example->save();*/
            $post_data = array();
            $post_data['total_amount'] =$a; # You cant not pay less than 10
            ...
            if (!is_array($payment_options)) {
                ...
                $payment_options = array();
                     ...
                     return redirect('amount')->with('success', true);
            }
    ...
    public function success(Request $request) 
    {
       // echo "Transaction is Successful";
        ...
        $email=$user->email;
        $phone=$user->phone;
        $payment_type="iwja membership fee";
        $a=$tran_id;
        $amount=$request->input('amount');
           $payment=new Payment;
        $payment->name=$name;
     ...
     /*   $user=auth()->user();
        ...
        ///$membership_status="Waiting for Admin Approval";
        $payment=new Payment;
        $payment->name=$name;
        $payment->email=$email;
        ...
        $payment->save();
        ///$payment->membership_status=$membership_status;
       

        ///echo "Done";
        return redirect('payment-confirm')->with('success', true);
        ...
        */
        ...
        if(isset($order_detials->order_status) && $order_detials->order_status=='Pending')
        {
        ...
        else if(isset($order_detials->order_status)=='Processing' || isset($order_detials->order_status)=='Complete')
        {
            /*
     ...
     public function cancel(Request $request) 
    {
      /*  $tran_id = $_SESSION['payment_values']['tran_id'];
        ...
        if(isset($order_detials->order_status)=='Pending')
        {
            $update_product = DB::table('orders')
                            ->where('order_id', $tran_id)
      ...
      if($request->input('tran_id')) #Check transation id is posted or not.
      {
        ...
        #Check order status in order tabel against the transaction id or order id.
          /*$order_detials = DB::table('orders')
                            ...
                            ->where('transaction_id', $tran_id)
                            ->select('name', 'email')->first();
                if(isset($order_details->order_status) =='Pending')
                {
                    $sslc = new SSLCommerz();
                ...
                else if(isset($order_details->order_status) == 'Processing' || isset($order_details->order_status) =='Complete')
                {
